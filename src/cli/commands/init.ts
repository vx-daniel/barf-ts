...
export async function initCommand(
  ...
  if (config.issueProvider === 'local') {
    ...
    logger.info(
      { issuesDir: config.issuesDir, planDir: config.planDir },
      'Directories created',
    )
  }
  ...
  if (config.issueProvider === 'github') {
    ...
    const BARF_LABELS = [
      {
        name: 'barf:new',
        color: 'e4e669',
        description: 'Issue not yet planned',
      },
      {
        name: 'barf:planned',
        color: 'bfd4f2',
        description: 'Issue planned, ready for build',
      },
      {
        name: 'barf:in-progress',
        color: 'fef2c0',
        description: 'Issue being worked on by barf',
      },
      {
        name: 'barf:stuck',
        color: 'e11d48',
        description: 'Issue blocked, needs intervention',
      },
      {
        name: 'barf:split',
        color: 'c5def5',
        description: 'Issue split into child issues',
      },
      {
        name: 'barf:completed',
        color: '0e8a16',
      ...
      },
      {
        name: 'barf:locked',
        color: 'd93f0b',
        description: 'Issue currently being processed',
      },
    ]
    ...
    for (const label of BARF_LABELS) {
      ...
      if (result.status !== 0 && !alreadyExists) {
        logger.error(
          { label: label.name, stderr: result.stderr.trim() },
